...
export function getGTOPreflopAdvice(
  ...
  if (stackBBs <= 20) {
    ...
    const mixNote = pf.isMixed
      ...
      : '';
    if (pf.action === 'PUSH') {
      return { action: 'RAISE', reason: `PUSH ALL-IN (${stackBBs}BB, Nash: топ ${pf.threshold.toFixed(0)}% рук)${mixNote}`, strength, frequencies: freqs };
    }
    if (pf.action === 'CALL') {
      return { action: 'CALL', reason: `CALL шов (${stackBBs}BB, Nash: топ ${pf.threshold.toFixed(0)}% рук)${mixNote}`, strength, frequencies: freqs };
    }
    return { action: 'FOLD', reason: `FOLD — вне Nash диапазона для ${position} при ${stackBBs}BB (Nash: топ ${pf.threshold.toFixed(0)}%)${mixNote}`, strength, frequencies: freqs };
  }
...
export function getFullAdvice(
  ...
  if (isPreflop) {
    ...
    const preflopAdvice = getGTOPreflopAdvice(holeCards, pos, betToCall > 0, stackBBs);
